...
public class BallManager {
	...
	*/
	...
	private BoostTarget turbo1 = new BoostTarget(400.0,200.0,50.0,50.0);
	private BoostTarget turbo2 = new BoostTarget(400.0,500.0,50.0,50.0);
	private DuplicatorTarget duplica = new DuplicatorTarget(400.0,350.0,50.0,50.0); 
	private double ballSpeed;
	private long t;
	private boolean turbo = false;
	private long inicioTurbo = 0;
	private ArrayList<Long> timeExtraBalls = new ArrayList<>();
	private ArrayList<IBall> balls = new ArrayList<>();
	private Queue<Long> timeTurboBalls = new LinkedList<>();
	private Queue<IBall> turboBalls = new LinkedList<>();
	...
	*/
	...
	public void draw(){ 
		balls.get(0).draw();
		if(!timeExtraBalls.isEmpty()) for(int i=1; i<balls.size(); i++) balls.get(i).draw();
	}
	...
	*/
	...
	public void update(long delta){ 
		balls.get(0).update(delta);
		if(!timeExtraBalls.isEmpty()) for(int i=1; i<balls.size(); i++) balls.get(i).update(delta);
	...
	*/
	...
	public int checkCollision(Wall wall){
		...
		if(balls.get(0).checkCollision(wall)) hits++;
		if(!timeExtraBalls.isEmpty()) {
			for(int i=1; i<balls.size(); i++) if(balls.get(i).checkCollision(wall)) hits++;
		}
		return hits;
	}
	...
	*/
	...
	public void checkCollision(Player player){
		balls.get(0).checkCollision(player);
		if(!timeExtraBalls.isEmpty()) for(int i=1; i<balls.size(); i++) balls.get(i).checkCollision(player);
	}
	...
	*/
	...
	public void checkCollision(Target target){		
		for(int i=0; i<balls.size();i++) {
			if(balls.get(i).checkCollision(target)) {
			if(target.getCy() == 500.0 || target.getCy() == 200.0){
				balls.get(i).setSpeed(ballSpeed * turbo1.BOOST_FACTOR);
				timeTurboBalls.add(t + turbo1.BOOST_DURATION);
				turboBalls.add(balls.get(i));
			}	
			if(target.getCy() == 350.0) {
				timeExtraBalls.add(t);
				balls.add(createBallInstance(balls.get(i).getCx(), balls.get(i).getCy(), balls.get(i).getWidth(), balls.get(i).getHeight(), Color.RED, balls.get(i).getSpeed(), balls.get(i).getVx(), balls.get(i).getVy() + Math.random() * 2));
			}
			 	
		}
		}
		
		if(!timeExtraBalls.isEmpty()) if(t >= timeExtraBalls.get(0) + duplica.EXTRA_BALL_DURATION) {
			timeExtraBalls.remove(0);
			balls.remove(1);
		}
		 if(!timeTurboBalls.isEmpty()){ 
			if (t >= timeTurboBalls.peek()) {
				turboBalls.poll().setSpeed(ballSpeed);
				timeTurboBalls.poll();
			}
		}		
